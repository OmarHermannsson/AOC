...
count = 0
for y, line in enumerate(tile_grid):    
    for x, col in enumerate(line):
        if col.pipe_type in "|JL" and col.distance == 0:
            col.pipe_type = '.'
    ...
    for x, col in enumerate(line):
        if col.pipe_type in "|JL" and col.distance == 0:
            col.pipe_type = '.'
        if col.pipe_type != 'S' and col.distance == 0 and len(re.findall('[|JLS]', str_line[:x])) % 2 != 0:
            #print(col, str_line[:x], x, re.findall('[|JL]', str_line[:x]))
            print(col, end=' ')
            count += 1
    print("]")
